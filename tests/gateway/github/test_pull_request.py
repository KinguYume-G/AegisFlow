...
class Authorizer:
    ...
    async def verify(self, authorization: WriteAuthorization, digest: str) -> None:
        if not self.allowed or digest != authorization.content_digest:
...
def authorization(changes: tuple[FileChange, ...]) -> WriteAuthorization:
    return WriteAuthorization(
        approval_id=uuid4(),
        tenant_id=uuid4(),
        run_id=uuid4(),
        step_id=uuid4(),
        repository_target=RepositoryTarget("owner", "fixture"),
        base_sha="a" * 40,
